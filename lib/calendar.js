...
export const convertDateStrToDid = (dateStr) => {
  const [year, month, date] = ymdFromDate(new Date(dateStr))
  return convertYmdToDid(year, month, date)
}

export const convertYmdToDid = (year, month, date) => {
  return (
    year.toString() +
    month.toLocaleString('en-US', {
      minimumIntegerDigits: 2,
...
export const convertDateToDid = (dt) => {
  const [year, month, date] = ymdFromDate(new Date(dt))
  return convertYmdToDid(year, month, date)
}

export const didToYmd = (did) => {
  return [
    parseInt(did.slice(0, 4)),
    parseInt(did.slice(4, 6)),
